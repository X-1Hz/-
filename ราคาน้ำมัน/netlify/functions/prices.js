...
async function scrapeFuelPrices() {
  const response = await axios.get(BOI_URL, {
    timeout: 9000, // Netlify functions timeout is 10s — leave 1s buffer
    headers: {
      "User-Agent":
  ...
  // Find the table that contains fuel price data
  let fuelTable = null;
  $("table").each((_, table) => {
  ...
  // Find header row and map station → column index
  let headerRow    = null;
  let stationCols  = {};
  ...
  // Extract prices for each station/fuel
  const priceMap = {};
  ...
  // Build final stations array (exclude susco_dealers if same as susco)
...
exports.handler = async (event, context) => {
  ...
  const headers = {
    ...
    // Cache the response on Netlify CDN for 30 minutes
